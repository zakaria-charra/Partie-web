...
export default class RestaurantsEdit extends Component {
  state={
    progress:0,
  ...
  delete(id){
    firebase.database().ref("services").child("Restaurants").child(id).remove().then(() => {
    ...
    render() {
        return (
            ...
            <div className="row hidden" id="imgs" style={{marginLeft:"28%"}}>
         ...
         <div className="alert alert-success hidden"  id="alertS">
             <strong>Success!</strong> <h6 id="success"> Restaurant modifié avec success</h6>
        </div>
   ...
   <div className="row"> 
         ...
         <div className="form-group">
           ...
           <div className="col-xs-6">
           <input type="tel"  pattern="^\+212 [0-9]{3}-[0-9]{6}" className="form-control" id="Fax"  placeholder="+212 000 000000" onChange={this.onChangeFax} value={this.state.fax} required/>
           </div>
         </div>
    ...
    <div className="row mt-3">
             <button type="button" class="btn btn-danger btn-block " onClick={this.delete.bind(this,this.state.id)} style={{marginRight:"55px"}}>Supprimer</button>
    </div> 
